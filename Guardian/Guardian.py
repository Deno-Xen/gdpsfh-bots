...
suggestions_channels = [745331693396820090, # suggestions
                        ...
                        836945466569523231, # giveaway-suggestion
...
async def on_message(message):
    ...
    for mentions in message.mentions:
        if mentions.id == 195598321501470720:
            if str(867129581957414963) in str(message.author.roles):
                return
            if message.author.id == 195598321501470720 or message.author.id == 180790976128745472:
                return
            log_channel = client.get_channel(867138503874707466)
            gulag_channel = client.get_channel(867132036400939008)
            user = message.author
            embed = discord.Embed(description=f"**{message.author.name}** was sent to the gulag for pinging an owner", color=discord.Colour.red())
            await message.channel.send(embed=embed)
            gulag_role = discord.utils.get(message.guild.roles, id=867129581957414963)
            member_role = discord.utils.get(message.guild.roles, id=743032231995506778)
            await user.add_roles(gulag_role)
            await user.remove_roles(member_role)
            await gulag_channel.send(f"<@{message.author.id}> you are now in the gulag for 5 minutes for pinging an owner. Enjoy your stay!")
            await log_channel.send(f"<@{message.author.id}> was sent to the gulag.")
            await asyncio.sleep(300)
            await user.add_roles(member_role)
            await user.remove_roles(gulag_role)
            return
